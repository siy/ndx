...
- **Issue tracker.** New top-level `ndx issue ...` command set: `add`, `list`, `show`, `update`, `close`, `reopen`, `rm`, `milestones`. Issues are drawers in the reserved `_issues_` room — they reuse drawer machinery wholesale (search, BM25, links, importance, mining). Status (`open` / `closed`), `closed_at`, and `milestone` live under namespaced `meta["issue.*"]` keys; the existing drawer `metadata` map absorbs both today's and future issue attributes (tags, etc.) without schema migrations. `close` stamps the closed-at timestamp and appends a structured trailer to the drawer text (`**Closed YYYY-MM-DD** — fix. Commit: sha.`) — fully searchable; reopening keeps the trailer as audit history. Optional `--link-drawer N` on close records a `derived_from` edge to a rationale drawer. `ndx issue milestones` aggregates open/closed counts per milestone. `ndx recall status` adds `Open issues: N` (plain) / `open_issues` (JSON). New `Palace::patch_drawer_metadata`, `append_drawer_text`, and `outgoing_links` helpers; classify and chore skill texts now call out `_issues_` as a CLI-managed reserved room.
